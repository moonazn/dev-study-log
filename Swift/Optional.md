...
- Swift는 기본적으로 ARC를 사용하여 메모리를 관리하지만, 불필요한 복사와 성능 저하를 방지하기 위해 소유권 시스템을 도입하였다.
  ...
  - ==🟢[값 타입(struct)에서 불필요한 복사 방지가 중요 -> 값 타입을 복사하지 않고 consume 사용 시 **메모리와 성능 최적화 가능**](https://github.com/moonazn/dev-study-log/blob/main/Swift/Optional.md#왜-값-타입을-복사하지-않고-consume을-사용하면-메모리와-성능-최적화가-될까-똑같이-기존-변수는-사용하지-못하게-되는-것-아닌가)==
